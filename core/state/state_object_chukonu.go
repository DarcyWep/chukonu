...
func (s *ChuKoNuStateObject) GetCommittedState(db Database, key common.Hash) common.Hash {
	...
	if s.db.snap == nil || err != nil {
		...
		if err != nil {
			s.db.setError(err)
			s.setOriginStableStorage(key, common.Hash{})
			return common.Hash{}
		}
	...
	s.setOriginStableStorage(key, value)
	return value
}
...
func (s *ChuKoNuStateObject) OriginState(db Database, key common.Hash) common.Hash {
	...
	val, cached := s.originStableStorage[key]
...
func (s *ChuKoNuStateObject) setOriginStableStorage(key, val common.Hash) {
	s.storageMutex.Lock()
	s.originStableStorage[key] = val
