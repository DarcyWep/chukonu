...
func (p *ChuKoNuFastProcessor) ChuKoNuFast(block *types.Block, statedb *state.ChuKoNuStateDB, cfg vm.Config) {
	startTime := time.Now()
	...
	accountLen := p.accountConflictDetection(statedb) // 依赖队列的长度
	//fmt.Println(accountLen)
	...
	fmt.Println(root, "ChuKoNu Fast", float64(block.Transactions().Len())/time.Since(startTime).Seconds())
}

func (p *ChuKoNuFastProcessor) accountConflictDetection(statedb *state.ChuKoNuStateDB) int {
...
func (p *ChuKoNuFastProcessor) slotConflictDetection(accountInfoCh chan *accountInfo, wg *sync.WaitGroup, statedb *state.ChuKoNuStateDB) {
	...
	for accInfo := range accountInfoCh {
		...
		ok := accInfo.atomicCounter.CompareAndSwap(false, true) // 首次权限开始授予
		if !ok {
			fmt.Printf("first rights granted failed")
		}
		for _, txIndex := range accInfo.readyTxs {
			rwSet := (*p.cknTxs[txIndex].tx.AccessPre)[accInfo.address]
			slots := make([]common.Hash, len(*rwSet.Slots))
			for key, _ := range *rwSet.Slots {
				slots = append(slots, key)
			}
			p.schedulerCh[txIndex%p.schedulerNumCPU] <- newToken(accInfo.address, accInfo, p.cknTxs[txIndex],
				state.NewStateTokenToTxDB(accInfo.address, accInfo.accountState, slots, accInfo.accountState.Deleted()),
				rwSet)
		}
		ok = accInfo.atomicCounter.CompareAndSwap(true, false) // 首次权限开始授予
		if !ok {
	...
	for token := range p.tokenManagerCh {
		...
		if len(grantedTxIndexes) > 0 { // 获取完了所有状态的事务，可以分发权限
			...
			for _, txIndex := range token.account.readyTxs {
				...
				p.schedulerCh[txIndex%p.schedulerNumCPU] <- newToken(token.account.address, token.account, p.cknTxs[txIndex],
