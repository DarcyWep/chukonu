...
func (cq *QueueGraph) QueuesSort() []string {
	...
	*/
	...
	sort.Strings(sortedStrings) // 地址排序

	for i, s := range sortedStrings {
		sortedQueues[s] = i // 地址ID
	}
...
func (cq *QueueGraph) GetAbortedNums() (int, []common.Hash) {
	count := 0
	abortHashes := make([]common.Hash, 0)
	edges := cq.Edges
	for k := range edges {
		if edges[k].isAborted {
			count++
			abortHashes = append(abortHashes, edges[k].set[0].TransInfo.Hash)
			//for _, hash := range edges[k].set {
			//	fmt.Println(hash.TransInfo.Hash)
			//}
			//fmt.Println()
		}
	}

	return count, abortHashes
}
