...
func compareRW() {
	...
	if name == "wuep" {
		rawConfig.Path = "/home/wuep/chukonu/data/data/ethereumdata/copchaincopy"
		rawConfig.Ancient = "/home/wuep/chukonu/data/data/ethereumdata/copchaincopy/ancient"
		stateConfig.Journal = "/home/wuep/chukonu/data/data/ethereumdata/triecache"
	} else {
	...
	for i := min; i.Cmp(max) == -1; i = i.Add(i, addSpan) {
		...
		if txsLen >= testTxsLen { // 对比 testTxsLen 个交易
			slotInconsistency := compareAccess(&accessAddrNormal, &accessAddrChu, txs)
			...
			slotInconsistencySum += slotInconsistency
			count += 1
			if count >= 10 {
				txSum := testTxsLen * count
				fmt.Println("不正确的事务比例：", (float64(slotInconsistencySum)/float64(txSum))*100)
				count = 0
				break
			}
		}
...
func compareAccess(accessAddrNormal *[]*types.AccessAddressMap, accessAddrChu *[]*types.AccessAddressMap, txs types.Transactions) int {
	...
	//	fmt.Println()
	//}
	//inconsistency := addrInconsistency + slotInconsistency - len(identicalTxs)
	//return inconsistency
	return slotInconsistency
}
